...
export default function InventoryPage() {
    ...
    const [rowsPerPage, setRowsPerPage] = useState<number>(5) // usar -1 para "Todos"
        ...
        <Container maxWidth="lg" sx={{py: 4}}>
            <Stack direction={{xs: 'column', sm: 'row'}} alignItems={{xs: 'stretch', sm: 'center'}}
                ...
                <Stack direction="row" spacing={1}>
                    <Button variant="contained" startIcon={<AddIcon/>} onClick={() => setOpen(true)}>Agregar
                        producto</Button>
                </Stack>
            </Stack>
            ...
            <Card sx={{p: 2, mb: 2}}>
                <Stack direction={{xs: 'column', md: 'row'}} spacing={2}>
                    ...
                    <FormControl fullWidth>
                        ...
                        <Select value={category} label="Categoría" onChange={(e) => {
                    ...
                    <FormControl fullWidth>
                        ...
                        <Select value={status} label="Estado" onChange={(e) => {
            ...
            <Card>
                    ...
                    <Table size="medium">
                        <TableHead>
                            <TableRow>
                                <TableCell onClick={() => handleSort('sku')}
                                    ...
                                    SKU {orderBy === 'sku' ? (order === 'asc' ? <ArrowUpwardIcon fontSize="inherit"/> :
                                    <ArrowDownwardIcon fontSize="inherit"/>) : null}
                                </TableCell>
                                ...
                                <TableCell align="right" onClick={() => handleSort('stock')}
                                           sx={{cursor: 'pointer', whiteSpace: 'nowrap'}}>
                                    Stock {orderBy === 'stock' ? (order === 'asc' ?
                                    <ArrowUpwardIcon fontSize="inherit"/> :
                                    <ArrowDownwardIcon fontSize="inherit"/>) : null}
                                </TableCell>
                                <TableCell align="right" onClick={() => handleSort('minStock')}
                                           sx={{cursor: 'pointer', whiteSpace: 'nowrap'}}>
                                    Mínimo {orderBy === 'minStock' ? (order === 'asc' ?
                                    <ArrowUpwardIcon fontSize="inherit"/> :
                                    <ArrowDownwardIcon fontSize="inherit"/>) : null}
                                </TableCell>
                                <TableCell align="right" onClick={() => handleSort('price')}
                                           sx={{cursor: 'pointer', whiteSpace: 'nowrap'}}>
                        ...
                        <TableBody>
                            {paged.map(r => {
                                ...
                                return (
                                    <TableRow key={r.id} hover>
                                        ...
                                        <TableCell
                                            align="right">${new Intl.NumberFormat('es-CL').format(r.price)}</TableCell>
                            ...
                            {paged.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={7}>
                                        <Box sx={{py: 6, textAlign: 'center', color: 'text.secondary'}}>Sin
                                            resultados</Box>
                                    </TableCell>
                                </TableRow>
                            )}
                ...
                <TablePagination
                    component="div"
                    count={sorted.length}
                    page={rowsPerPage === -1 ? 0 : page} // si es "Todos", fijar página 0
                    ...
                    // Texto de rango cuando es "Todos"
